 ...
 */
export interface ISchemaServerProvder extends IAppSchemaProvider
{
  /**
   ...
   */
  saveSchema(schema: NodeSchema): Promise<boolean>
   ...
   * @returns message the error message if provided
   */
  saveEnumSubList(schema: string, value: any, values: Entry<string>[], append?: boolean): Promise<boolean>

  /**
   * Save the app schema to the server
   * @param app the app schema to save
   */
  saveAppSchema(app: AppSchema): Promise<boolean>
   ...
   */
  saveAppFieldSchema(app: string, field: AppFieldSchema): Promise<boolean>
   ...
   */
  saveAppWorkflowSchema(app: string, schema: AppWorkflowSchema): Promise<boolean>
...
const defaultSchemaServerProvider: ISchemaServerProvder = {
  ...
  deleteSchema: async (name: string): Promise<boolean> => {
    return (await postSchemaApi("/delete-schema", {name}))?.result
  },

  saveEnumSubList: async (name: string, value: any, values: Entry<string>[], append?: boolean): Promise<boolean> => {
    return (await postSchemaApi("/save-enum-sub-list", { name, value, values, append }))?.result
  },

  saveAppSchema: async function (schema: AppSchema): Promise<boolean> {
    return (await postSchemaApi("/save-app-schema", { schema }))?.result
  },

  deleteAppSchema: async function (app: string): Promise<boolean> {
    return (await postSchemaApi("/delete-app-schema", {app}))?.result
  },

  saveAppFieldSchema: async function (app: string, schema: AppFieldSchema): Promise<boolean> {
    return (await postSchemaApi("/save-app-field-schema", { app, schema }))?.result
  },

  deleteAppFieldSchema: async function (app: string, field: string): Promise<boolean> {
    return (await postSchemaApi("/delete-app-field-schema", { app, field }))?.result
  },

  swapAppFieldSchema: async function (app: string, field: string, other: string): Promise<boolean> {
    return (await postSchemaApi("/swap-app-field-schema", { app, field, other }))?.result
  },

  saveAppWorkflowSchema: async function (app: string, schema: AppWorkflowSchema): Promise<boolean> {
    return (await postSchemaApi("/save-app-workflow-schema", { app, schema }))?.result
  },
  
  deleteAppWorkflowSchema: async function (app: string, workflow: string): Promise<boolean> {
    return (await postSchemaApi("/delete-app-workflow-schema", { app, workflow }))?.result
  },

  toggleAppWorkflowSchema: async function (app: string, workflow: string, active: boolean): Promise<boolean> {
    return (await postSchemaApi("/toggle-app-workflow-schema", { app, workflow, active }))?.result
  }
}
