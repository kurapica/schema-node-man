<template>
    <section>
        ...
        <el-collapse accordion>
            <el-collapse-item title="本次开源内容">
                <markdown>
                    本次开源的部分是剥离后端后，仅前端专用的部分，包含两个库，和一个前端类型配置管理器。

                    1. schema-node: 无依赖的类型数据节点系统，基于数据类型生产数据节点，并基于各自的关系实现数据订阅。

                    2. schema-node-vue-view: 基于Vue3和ElmentPlus实现的数据节点视图库，我们可以为数据类型注册不同的视图实现，这里提供的是默认实现。

                    3. schema-node-man: 前端用类型配置管理器(暂不开源)，可以上传，下载自定义类型，可以自动缓存。类型下载后可以配合前两个库在各个项目中使用。
                </markdown>
            </el-collapse-item>
            <el-collapse-item title="应用和服务器架构-非本次开源内容">
                <markdown>
                    ...
                    客户端可以使用统一的提交数据接口将应用的变更数据提交给服务器，除了保存变更外，应用字段间可以配置关系，常用于将数据进行过滤，转换之后进行结果汇总。
