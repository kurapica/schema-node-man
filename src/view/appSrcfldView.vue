<template>
  ...
  <el-select v-else v-model="data" style="width: 100%;min-width: 120px;" :disabled="state.readonly || state.disable"
    :clearable="!state.require" :placeholder="scalarNode.selectPlaceHolder">
    <el-option v-for="item in state.whiteList" :key="typeof (item) === 'object' ? item.value : item"
      :label="typeof (item) === 'object' ? item.label : item" :value="typeof (item) === 'object' ? item.value : item">
    </el-option>
  </el-select>
</template>

<script lang="ts" setup>
import { type AnySchemaNode, _L, getAppSchema, isNull, isSchemaCanBeUseAs, ScalarNode, StructNode, type IAppSchema, ArrayNode } from 'schema-node'
